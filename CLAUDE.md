...
> Last updated: US00133 (lib/image-meta.ts — readImageSize + assertMinShortSide + MIN_COVER_IMAGE_SHORT_SIDE_PX; lib/posts.ts calls assertMinShortSide per post; lib/seo.ts's buildPageMetadata resolves real og:image dimensions; lib/image-meta.test.ts + lib/posts.test.ts; F0013)
...
│   ├── posts.test.ts    # Vitest over getAllPosts(): every coverImage resolves ≥600px, no two posts share a coverImage, no post references a deleted orphan placeholder (US00133)
