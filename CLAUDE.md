...
│   ├── deal-roundup/       # F0015 weekly deal-roundup pipeline (US00151) — sibling of ingest/, shares only the data/deals/ input
...
- **Every published post must clear `MIN_POST_WORDS` (800, counted by `countWords()` in `lib/format.ts`), carry ≥2 `h2` sections, embed ≥1 `<ProductCard slug>`, and have non-placeholder frontmatter with a genuine 50–160-char `summary` and non-empty `tags`.** Enforced by `lib/posts.test.ts` (`getAllPosts() publish-grade depth (US00134)`), which fails naming the offending post slug and its word count — with **no exemption list**; a genuinely short format needs a documented exemption added deliberately, never a silently lowered threshold (US00134/F0013).
