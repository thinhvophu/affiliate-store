...
│   ├── analytics.ts     # resolveGaMeasurementId() — the one place that decides whether GA4 loads; **server-only** (lib/env.ts reads `process.env[name]` dynamically, which Next.js does not inline into client bundles), so callers resolve it in a Server Component and pass the ID down as a prop; returns undefined with no ID or outside `NODE_ENV=production`, throws on a malformed ID (US00143)
│   ├── analytics.test.ts # Vitest — unset/empty ID, valid ID in production, ID present but non-production NODE_ENV (dev/test), malformed ID throws, injection-shaped ID throws; manipulates process.env via `vi.stubEnv` + `vi.resetModules()` per case (US00143)
