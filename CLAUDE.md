...
│       ├── args.ts         # parseIngestArgs() — hand-rolled `--key=value` / `--flag` CLI parser; `--query`/`--count`/`--date` for `--source=scrape` (US00121, US00124)
...
│       └── README.md       # Operator doc: flags, dry-run, exit codes, slug/dedupe rules, image-staging rules, two-step scrape flow (US00121, US00122, US00123, US00124)
├── data/
