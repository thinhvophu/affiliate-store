...
│   ├── scaffold-post.ts    # `scaffold:post` CLI entry: parse `--category --products --title? --slug?` → assertCategoryRegistered → resolve each product via getProductBySlug (fatal on unknown slug, before any write) → compute post slug (fatal if content/posts/<slug>.mdx already exists) → coverImage = first product's images[0] → renderPostStub() → write content/posts/<slug>.mdx (US00126)
│   ├── scaffold/
...
- **The `scaffold:post` CLI (`scripts/scaffold-post.ts`, F0012/US00126) is a separate entry point from `ingest:products`** — different inputs (a category + 1–2 already-ingested product slugs) and a different output (`content/posts/<slug>.mdx`, not a product fixture). It pre-validates every named product slug via `getProductBySlug` and exits non-zero naming the first unknown one **before writing any file** (mirrors `<MdxProductCard>`'s build-time throw on an unresolved embed, just surfaced earlier). `coverImage` in the written stub is always the **first** named product's `images[0]`, so two scaffolds naming different products get different cover images by construction. Frontmatter `title`/`summary` are always literal `"TODO: …"` placeholders — a human writes the real copy and prose body before publishing; `--title`/`--slug` only influence the output **filename**, not the frontmatter `title`. Reuses `assertCategoryRegistered` (`lib/categories.ts`) and `slugifyProductName` (`scripts/ingest/slug.ts`, US00122) rather than re-implementing either. Refuses to overwrite an existing `content/posts/<slug>.mdx`.
