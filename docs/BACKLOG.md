...
- **US00133** — ✅ Done (revised). The 5 posts carrying the 1×1 / 332-byte placeholder covers (`logitech-g102-lightsync.jpg`, `keychron-k2-v2.jpg` — orphans from the mock-product cleanup) were deleted outright (2026-08-25 decision, superseding the original "replace the covers" scope), which removed the orphan image files in the same change. Still open: the build-time assertion in `lib/posts.ts` that a post's `coverImage` resolves to a real, non-degenerate file — same fail-loudly pattern as `assertAffiliateUrl` / `assertCategoryRegistered`
- **US00134** — rewrite the **3 remaining originally-audited posts** (`ban-phim-co-cho-nguoi-moi`, `top-5-chuot-gaming-gia-re`, `danh-gia-chuot-gaming-g305-deathadder`) to ≥800 words, each carrying ≥1 `<ProductCard>` embed. Scope narrowed from "all 8" by the 2026-08-25 decision to delete the 5 thin posts instead of rewriting them (docs/specs/F0013.md); the 2026-08-22 no-exemption rationale still applies to the 3 that remain
