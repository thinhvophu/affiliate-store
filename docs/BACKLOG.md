...
- **US00135** — 🟡 In Progress. **Domain registered 2026-09-02: `muagear.com`.** Half A (repo hygiene, `verify:canonical` script, branding rename) is up as PR `feat/US00135`: `lib/site.ts`'s `SITE_NAME` and `package.json`'s `"name"` renamed `"aff-store"` → `"MuaGear"`/`"muagear"`, plus every other rendered surface that had it hardcoded (Footer copyright, About/Privacy/Affiliate-Disclosure page copy, category intros in `lib/categories.ts`) — the plan's original assumption that `SITE_NAME` was already a single chokepoint was wrong. `scripts/verify-canonical.ts` (`npm run verify:canonical -- --base=<url>`) exists, is unit-tested, and was confirmed to correctly fail against a local build still carrying the `example.com` placeholder. Half B (Vercel Production env var → `https://muagear.com`, redeploy, live `verify:canonical` run) is the remaining gate before this story can be marked Done — see `docs/plans/US00135.md` §6. Plan: `docs/plans/US00135.md`
