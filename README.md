...
│   ├── deal-roundup/     # F0015 weekly deal-roundup data-input layer (US00146) — RankedDeal/DroppedDeal model, loadRankedDeals() (reads data/deals/<date>.json, never re-sorts/re-scores), CLI arg parser, summary reporter; sibling of ingest/, shares only the data/deals/ input
...
- **The deal-roundup source (F0015/US00146) follows the identical read-only-file rule** — `scripts/deal-roundup/source.ts`'s `loadRankedDeals()` reads the same `data/deals/<date>.json`, sharing `scripts/ingest/sources/deal-schema.ts`'s `parseDealsSnapshot`/`RawDeal` rather than re-declaring the schema. The MCP call (`mcp__shopee-affiliate__scrape_products`) belongs to the `/deal-roundup` slash command (US00149), never to this Node process. It matches the exact (trimmed) `--query` keyword, takes the tool's top `--top` in tool order with no re-sort/re-filter/re-score, and drops (never throws for) a deal whose `affiliateUrl` is missing or fails `assertAffiliateUrl`, naming the reason via `printDealSummary()`. A shortfall against `--top` is a normal, exit-0 result. `scripts/deal-roundup/` is a sibling of `scripts/ingest/`, not a subdirectory — the two pipelines share only the `data/deals/` input.
