...
│   ├── analytics.ts     # resolveGaMeasurementId() — the one place that decides whether GA4 loads; server-only, returns undefined with no ID or outside `NODE_ENV=production`, throws on a malformed ID (US00143)
...
- **Analytics gating lives in one place: `lib/analytics.ts`.** No file outside it may read `NEXT_PUBLIC_GA_MEASUREMENT_ID` or decide whether analytics is enabled. It is server-only — the measurement ID is resolved in a Server Component and passed down as a prop; a `"use client"` module must never import it (US00143).
